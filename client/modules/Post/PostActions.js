...
export function thumbUpRequest(cuid, votes) {
  return (dispatch) => {
    return callApi(`post/thumbUp/${cuid}`, 'put', { votes: votes + 1 }).then(() => dispatch(thumbUp(cuid)));
  };
}
...
export function thumbDownRequest(cuid, votes) {
  return (dispatch) => {
    return callApi(`post/thumbDown/${cuid}`, 'put', { votes: votes - 1 }).then(() => dispatch(thumbDown(cuid)));
  };
}
